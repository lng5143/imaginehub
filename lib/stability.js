...
export const generateStabilityImages = async (data) => {
    ...
    const uploadRes = await uploadGeneratedImages({ images: [resData] });
    if (uploadRes.error) {
        return { error: "Failed to upload images", data: uploadRes }
    }
    
    const generationRes = await insertImageGeneration(data);
    if (generationRes.error) {
        return { error: "Failed to insert image generation", data: generationRes }
    }
